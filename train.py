...
def create_multi_train_val_dataloader(opt, logger):
    # create train and val dataloaders
    train_loader, val_loaders = None, []
    train_sets = []
    for phase, dataset_opt in opt['datasets'].items():
        if phase == "data_loader":
            continue
        if phase.split('_')[0] == 'train':
            dataset_enlarge_ratio = dataset_opt.get('dataset_enlarge_ratio', 1)
            train_set = build_dataset(dataset_opt)
    ...
    each_trainset_len = [len(train_sets[0])]
    for i in range(1,len(train_sets)):
        train_set += train_sets[i]
        each_trainset_len.append(len(train_sets[i]))

    data_loader_opt = opt['datasets']['data_loader']
    data_loader_opt["phase"] = "train"

    train_sampler = EnlargedSampler(train_set, opt['world_size'], 
                                    opt['rank'], data_loader_opt["dataset_enlarge_ratio"], 
    ...
    num_iter_per_epoch = math.ceil(
        len(train_set) * dataset_enlarge_ratio / (data_loader_opt['batch_size_per_gpu'] * opt['world_size']))
...
def train_pipeline(root_path, opt=None, args=None):
    ...
    if args.split_index:
        opt['split_index'] = args.split_index
        for phase, dataset_opt in opt['datasets'].items():
            opt['datasets'][phase]['split_index'] = args.split_index
    ...
    if "multi_dataset" in opt and opt["multi_dataset"]:
        result = create_multi_train_val_dataloader(opt, logger)
    else:
    ...
    if "multi_dataset" in opt and opt["multi_dataset"]:
        prefetch_mode = opt['datasets']['data_loader'].get('prefetch_mode')
    else:
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        while train_data is not None:
            ...
            # validation
            if ("val_type" in opt['val'] and opt['val']["val_type"] != "epoch") or  "val_type" not in opt['val']:
        ...
        # validation
        if "val_type" in opt['val'] and opt['val']["val_type"] == "epoch":
            if opt.get('val') is not None and ((epoch + 1) % opt['val']['val_freq_epoch'] == 0):
                logger.info(f'{len(val_loaders)} validation datasets are used for validation.')
                for val_loader in val_loaders:
                    model.validation(val_loader, current_iter, tb_logger, opt['val']['save_img'])
        # end of iter
